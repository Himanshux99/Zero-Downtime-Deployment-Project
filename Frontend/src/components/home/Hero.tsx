...
export default function HeroSection({
  ...
  useEffect(() => {
    ...
    const checkBackend = async () => {
      try {
        const backend = await axios.get("local/api/v1/healthCheck");
        console.log(backend.data);
        setApiVersion(backend.data?.version ?? null);
      } catch (err) {
        console.error(err);
      }
    };
        ...
        <div className="md:col-span-4 grid grid-cols-2 md:grid-cols-1 gap-3">
          ...
          >
            ...
            <div className="flex items-baseline gap-2">
              ...
              {apiVersion && (
                <span className="text-xs font-mono text-swiss-gray-400 border border-swiss-gray-200 rounded px-1.5 py-0.5 leading-none">
                  {apiVersion}
                </span>
              )}
            </div>
